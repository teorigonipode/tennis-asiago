  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-ink-900">Manutenzione</h1>
          <p className="text-sm text-ink-500">Chiudi campi per manutenzione o eventi.</p>
        </div>
            ...
            return (
              <div key={c.id} className="card flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-ink-800">{court?.name ?? 'Campo'}</p>
                    <p className="text-sm text-ink-500">
                      {formatDateShort(c.start_at)} → {formatDateShort(c.end_at)}
                    </p>
                    {c.reason && <p className="text-xs text-ink-400">{c.reason}</p>}
                  </div>
                </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md card p-6 animate-scaleIn">
        <h2 className="font-display text-lg font-bold text-ink-900 mb-4">Nuova chiusura</h2>
