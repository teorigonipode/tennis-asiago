...
import { COURT_SURFACES } from '@/lib/clubConfig';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-forest-900">Campi</h1>
          <p className="text-sm text-cream-500">Aggiungi, modifica o disattiva i campi.</p>
        </div>
          ...
          <div key={court.id} className="card overflow-hidden">
            ...
            <div className="p-4">
              <div className="flex items-start justify-between">
                ...
                <span className={`badge ${court.is_active ? 'bg-forest-100 text-forest-800' : 'bg-cream-100 text-cream-500'}`}>
                  {court.is_active ? 'Attivo' : 'Disattivato'}
                </span>
              </div>
              <p className="mt-1 text-xs text-cream-500">{court.surface} · {court.is_indoor ? 'Coperto' : 'Scoperto'}</p>
      ...
      {courts.length === 0 && (
        <div className="card p-8 text-center text-cream-500">
          <Home className="mx-auto mb-2 h-8 w-8 text-cream-300" />
      ...
      <div className="relative w-full max-w-lg card p-6 animate-scaleIn max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={form.surface} onChange={(e) => setForm({ ...form, surface: e.target.value })} className="input">
                {COURT_SURFACES.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
          ...
          <div>
            ...
            <input type="number" min="0" value={form.display_order ?? 0} onChange={(e) => setForm({ ...form, display_order: Number(e.target.value) })} className="input" />
          </div>
          <div className="flex flex-wrap gap-4">
            <label className="flex items-center gap-2 text-sm text-forest-700">
              <input type="checkbox" checked={form.is_indoor ?? false} onChange={(e) => setForm({ ...form, is_indoor: e.target.checked })} className="h-4 w-4 rounded border-cream-300 text-forest-600" />
              Campo coperto
            </label>
            <label className="flex items-center gap-2 text-sm text-forest-700">
              <input type="checkbox" checked={form.has_lighting ?? true} onChange={(e) => setForm({ ...form, has_lighting: e.target.checked })} className="h-4 w-4 rounded border-cream-300 text-forest-600" />
              Illuminazione
            </label>
            <label className="flex items-center gap-2 text-sm text-forest-700">
              <input type="checkbox" checked={form.is_active ?? true} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} className="h-4 w-4 rounded border-cream-300 text-forest-600" />
              Attivo
            </label>
          </div>
